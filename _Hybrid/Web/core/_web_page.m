...
@interface _WebPageConfiguration : WKWebViewConfiguration
@end

@implementation _WebPageConfiguration
...
@interface _WebPage ( PrivateMethod )
...
- (void)dealloc {
    if ([_webView isKindOfClass:[UIWebView class]]) {
		...
		if (self.delegate && [self.script respondsToSelector:@selector(webViewRegisterObjCMethodNameForJavaScriptInteraction)]) {
			__weak typeof(self) weakSelf = self;
			[[self.script webViewRegisterObjCMethodNameForJavaScriptInteraction] enumerateObjectsUsingBlock:
...
- (instancetype)initWithFrame:(CGRect)frame JSPerformer:(nonnull id)performer {
    if (self = [super initWithFrame:frame]) {
        ...
        if ([_WebBrowser sharedInstance].config.enabledWKWebView) {
...
- (void)setDataDetectorTypes:(_WebPageDataDetectorTypes)dataDetectorTypes {
    if ([_webView isKindOfClass:[UIWebView class]]){
...
- (BOOL)webView:(UIWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    ...
    if ([self.delegate respondsToSelector:@selector(webView:shouldStartLoadWithRequest:navigationType:)]) {
        return [self.delegate webView:self shouldStartLoadWithRequest:request navigationType:(_WebPageNavigationType)navigationType];
    }
    return YES;
}
...
- (void)webViewDidFinishLoad:(UIWebView *)webView; {
    ...
    if([self.script respondsToSelector:@selector(webViewRegisterObjCMethodNameForJavaScriptInteraction)]){
        __weak typeof(self) weakSelf = self;
        [[self.script webViewRegisterObjCMethodNameForJavaScriptInteraction] enumerateObjectsUsingBlock:
         ^(NSString * _Nonnull name, NSUInteger idx, BOOL * _Nonnull stop) {
             __strong typeof(weakSelf) strongSelf = weakSelf;
...
{  
    ...
    if ([self.delegate respondsToSelector:@selector(webView:shouldStartLoadWithRequest:navigationType:)])
    {
        isNavigator = [self.delegate webView:self shouldStartLoadWithRequest:navigationAction.request navigationType:(_WebPageNavigationType)(navigationAction.navigationType < 0? navigationAction.navigationType : 5)];
    }
...
- (void)configureWKWebViewWithFrame:(CGRect)frame {
    WKWebView * web = [[WKWebView alloc] initWithFrame:frame configuration:[[_WebPageConfiguration alloc] init]];
    web.UIDelegate = self;
...
@implementation _WebPage ( Navigation )
...
@implementation _WebPage ( PrivateMethod )
