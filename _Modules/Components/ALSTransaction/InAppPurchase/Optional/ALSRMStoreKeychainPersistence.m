//
//  ALSRMStoreKeychainPersistence.m
//  RMStore
//
...
static void RMKeychainSetValue(NSData *value, NSString *key)
{
    ...
    {
        NSLog(@"ALSRMStoreKeychainPersistence: failed to set key %@ with error %ld.", key, (long)status);
    }
}
...
static NSData* RMKeychainGetValue(NSString *key)
{
    ...
    {
        NSLog(@"ALSRMStoreKeychainPersistence: failed to get key %@ with error %ld.", key, (long)status);
    }
    return (__bridge NSData*)value;
}

@implementation ALSRMStoreKeychainPersistence {
...
- (NSDictionary*)transactionsDictionary
{
    ...
    { // Reading the keychain is slow so we cache its values in memory
        ...
        if (data)
        {
            ...
            if (!transactions)
            {
                NSLog(@"ALSRMStoreKeychainPersistence: failed to read JSON data with error %@", error);
            }
        }
...
- (void)setTransactionsDictionary:(NSDictionary*)dictionary
{
    ...
    if (dictionary)
    {
        ...
        if (!data)
        {
            NSLog(@"ALSRMStoreKeychainPersistence: failed to write JSON data with error %@", error);
        }
    }
