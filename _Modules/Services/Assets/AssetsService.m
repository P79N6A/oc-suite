...
- (BOOL)configWith:(NSString *)albumName {
    if (![self albumWithName:albumName]) {
...
#pragma mark - Album manage

- (BOOL)albumWithName:(NSString*)albumName {
    __block BOOL exists = NO;
    [self.assetsLibrary enumerateGroupsWithTypes:ALAssetsGroupAlbum
                                      usingBlock:^(ALAssetsGroup *group, BOOL *stop) {
                               if ([[group valueForProperty:ALAssetsGroupPropertyName] isEqualToString:albumName]) {
                                   LOG(@"<VideoAlbumManager> Album '%@' found", albumName);
                                   exists = YES;
                               }
                           } failureBlock:^(NSError *error) {
                             LOG(@"<VideoAlbumManager> Error enumerating the albums:\nError: %@", [error localizedDescription]);
                             exists = NO;
                           }];
    return exists;
}

- (void)addAlbumWithName:(NSString *)albumName {
    LOG(@"<VideoAlbumManager> Poging om album '%@' aan te maken", albumName);
    
    [self.assetsLibrary addAssetsGroupAlbumWithName:albumName
                                        resultBlock:^(ALAssetsGroup *group) {
                                 LOG(@"<VideoAlbumManager> Album '%@' created", albumName);
                             } failureBlock:^(NSError *error) {
                                LOG(@"<VideoAlbumManager> Error creating the album:\nError: %@", [error localizedDescription]);
                             }];
}

#pragma mark - Assets manage
...
@def_namespace( service, assets, AssetsService )
