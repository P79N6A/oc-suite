...
- (UIImage *)imageByApplyingBlurWithRadius:(CGFloat)blurRadius tintColor:(UIColor *)tintColor {
    return [self imageByApplyingBlurWithRadius:blurRadius tintColor:tintColor saturationDeltaFactor:1.8 maskImage:nil];
}
...
#define ENABLE_TINT                     1
    ...
    if (!self.CGImage)
    {
        NSLog(@"*** error: self must be backed by a CGImage: %@", self);
        return nil;
    }
    ...
    CGBitmapInfo selfBitmapInfo = CGImageGetBitmapInfo(inputCGImage);
    CGImageAlphaInfo selfAlphaInfo = (selfBitmapInfo & kCGBitmapAlphaInfoMask);
    ...
    if (selfAlphaInfo == kCGImageAlphaNone || selfAlphaInfo == kCGImageAlphaNoneSkipLast || selfAlphaInfo == kCGImageAlphaNoneSkipFirst)
    ...
    if (hasBlur || hasSaturationChange)
    {
        ...
        if (e != kvImageNoError)
        {
            NSLog(@"*** error: vImageBuffer_InitWithCGImage returned error code %zi for self: %@", e, self);
        ...
        if (hasBlur)
        {
            ...
            //
            CGFloat inputRadius = blurRadius * selfScale;
