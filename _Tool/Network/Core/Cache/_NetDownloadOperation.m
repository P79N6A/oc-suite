...
@interface _NetDownloadOperation ()  <NSURLConnectionDataDelegate>
...
@implementation _NetDownloadOperation
...
- (void)connectionDidFinishLoading: (NSURLConnection *) connection {
    ...
    switch (self.cacheableItem.info.statusCode) {
        ...
        default: {
            ...
            if (attr) {
                ...
                LOG(@"Failed to get file attributes for file at path %@. Error: %@", path, [error description]);
            }
            
            [self.cacheableItem flagAsDownloadFinishedWithContentLength:self.cacheableItem.info.contentLength];
            
            if (error) {
                LOG(@"Error while finishing download: %@", [error localizedDescription]);
            } else {
                // Only cache response if it has a validUntil date and only if we're not in offline mode.
                if (self.cacheableItem.validUntil) {
                    LOG(@"Updating file modification date for object with URL: %@", [self.cacheableItem.url absoluteString]);
    ...
    BOOL hasAlreadyReturnedCacheItem = (self.cacheableItem.hasReturnedCachedItemBeforeRevalidation && self.cacheableItem.cacheStatus == NetCacheStatusNotModified);
...
- (void)connection:(NSURLConnection*)connection didFailWithError:(NSError*)anError {
    ...
    if (connectionLostOrNoConnection) {
        ...
        LOG(@"ERROR in download operation: %@", anError);
    }
...
- (void)handleResponse:(NSURLResponse *)response {
    ...
    if (self.cacheableItem.cacheStatus == NetCacheStatusRevalidationPending) {
        switch (self.cacheableItem.info.statusCode) {
            case 304:
                self.cacheableItem.cacheStatus = NetCacheStatusNotModified;
                self.cacheableItem.validUntil = self.cacheableItem.info.expireDate;
                // The resource has not been modified, so we exit here
                return;
            case 200:
                self.cacheableItem.cacheStatus = NetCacheStatusModified;
                break;
        }
    } else {
        self.cacheableItem.info.responseTimestamp = [now timeIntervalSinceReferenceDate];
...
- (void)handleResponseHeaderFields:(NSDictionary *)headerFields now:(NSDate*) now {
    ...
    self.cacheableItem.info.serverDate = dateField ? [_NetDateParser gh_parseHTTP:dateField] : now;
    NSDate *lastModifiedDate = modifiedField ? [_NetDateParser gh_parseHTTP:modifiedField] : now;
    self.cacheableItem.info.lastModified = lastModifiedDate;
    self.cacheableItem.info.expireDate = [_NetDateParser gh_parseHTTP:expiresField];
