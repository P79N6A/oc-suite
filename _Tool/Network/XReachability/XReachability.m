...
NSString *const kXReachabilityChangedNotification = @"kXReachabilityChangedNotification";

NSString *const kRRVPNStatusChangedNotification = @"kRRVPNStatusChangedNotification";

@interface XReachability() {
    BOOL _vpnFlag;
}
...
- (void)startNotifier
{
    ...
    self.previousStatus = XStatusUnknown;
...
- (void)reachabilityWithBlock:(void (^)(ReachabilityStatus status))asyncHandler
{
    ...
    if ([GLocalConnection currentLocalConnectionStatus] == LC_UnReachable)
    {
        if (asyncHandler != nil)
        {
            asyncHandler(XStatusNotReachable);
        }
        return;
    }
             ...
             if (rtn == 0) // state changed & state available, post notification.
             {
                 if ([strongSelf.engine isCurrentStateAvailable])
                 {
                     ...
                     dispatch_async(dispatch_get_main_queue(), ^{
                         __strong __typeof(weakSelf)strongSelf = weakSelf;
                         [[NSNotificationCenter defaultCenter] postNotificationName:kXReachabilityChangedNotification
                                                                             object:strongSelf];
                     });
                 }
             }
...
- (ReachabilityStatus)currentReachabilityStatus
{
    ...
    switch (currentID)
    {
        ...
        {
            return XStatusNotReachable;
        }
        case RRStateWIFI:
        {
            return XStatusViaWiFi;
        }
        case RRStateWWAN:
        {
            return XStatusViaWWAN;
        }
        case RRStateLoading:
        {
...
- (NSString *)paramValueFromStatus:(LocalConnectionStatus)status
{
    switch (status)
    {
        ...
        default:
        {
            NSLog(@"XReachability error! paramValueFromStatus not matched!");
            return @"";
        }
    }
}
