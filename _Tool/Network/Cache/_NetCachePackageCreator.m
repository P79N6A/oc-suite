...
#import "_net_cache_package_creator.h"
#import "ZipArchive.h"

#define kDefaultMaxItemFileSize kAFCacheInfiniteFileSize

@implementation _NetCachePackageCreator
...
- (_NetCacheRequest *)newCacheableItemForFileAtPath:(NSString *)filepath
    ...
    NSURL *url;
    NSString *escapedUrlString = [_NetCacheRequest urlEncodeValue:filepath];
    if (baseURL) {
    ...
    _NetCacheRequest *item = [[_NetCacheRequest alloc] initWithURL:url lastModified:lastModified expireDate:expireDate];
...
#endif
    [_NetCache sharedInstance].maxItemFileSize = maxItemFileSize;
        ...
        [self enumerateFilesInFolder:folder processHiddenFiles:processHiddenFiles usingBlock: ^ (NSString *file, NSDictionary *fileAttributes) {
            ...
            if ([fileType isEqualToString:NSFileTypeRegular]) {
                if (!hidden || addAllFiles) {
                    ...
                    _NetCacheRequest *item = [self newCacheableItemForFileAtPath:file lastModified:lastModificationDate baseURL:baseURL maxAge:maxAge baseFolder:folder];
