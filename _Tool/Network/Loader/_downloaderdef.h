...
/**
 BackgroundSessionCompletionHandlerBlock is a block assigned to the HWIFileDownloader
 in the application delegate on call application:handleEventsForBackgroundURLSession:completionHandler:
 */
typedef void (^BackgroundSessionCompletionHandlerBlock)();
...
/**
 _DownloadDelegate is a protocol for handling salient download events.
 */
@protocol _DownloadDelegate
...
/**
 Start a download with DownloadStore
 */
- (void)startDownload;

/**
 Post process, like unzip
 */
- (void)beginPostprocess;

/**
 Get stored model from DownloadStore
 */
+ (nullable id)storedDownloadModelWithIdentifier:(nonnull NSString *)aDownloadIdentifier;

/**
 Validata data that the client received.
 */
