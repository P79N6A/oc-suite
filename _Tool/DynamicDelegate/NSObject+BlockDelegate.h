...
/** The A2BlockDelegate category extends features provided by _DynamicDelegate
 ...
 `_DynamicDelegate`. This behavior is documented in detail in the class
 documentation for _DynamicDelegate, and examples exist in BlocksKit.
 */
...
/** Synthesizes multiple properties and links them to the appropriate selector
 ...
 _DynamicDelegate assumes a protocol name `FooBarDataSource` for instances of
...
/** Synthesizes multiple properties and links them to the appropriate selector
 ...
 _DynamicDelegate assumes a protocol name `FooBarDelegate` for instances of
 class `FooBar`. The method will generate appropriate `setHandler:` and
 `handler` implementations for each property name and selector name string pair.
 ...
 */
@property (NS_NONATOMIC_IOSONLY, readonly, strong) id ensuredDynamicDelegate;
