...
#import "_BlockInvocation.h"

Protocol *__dataSourceProtocol(Class cls);
Protocol *__delegateProtocol(Class cls);
Protocol *__protocolForDelegatingObject(id obj, Protocol *protocol);
...
@interface A2DynamicClassDelegate : _DynamicDelegate
...
@implementation _DynamicDelegate
...
- (Class)class {
	...
	if (myClass == [_DynamicDelegate class] || [myClass superclass] == [_DynamicDelegate class])
...
- (NSMethodSignature *)methodSignatureForSelector:(SEL)aSelector {
	_BlockInvocation *invocation = nil;
...
+ (NSString *)description {
	return @"_DynamicDelegate";
}
    
- (NSString *)description {
	return [NSString stringWithFormat:@"<_DynamicDelegate:%p; protocol = %@>", (__bridge void *)self, NSStringFromProtocol(self.protocol)];
}

- (void)forwardInvocation:(NSInvocation *)outerInv {
	SEL selector = outerInv.selector;
	_BlockInvocation *innerInv = nil;
...
- (void)implementMethod:(SEL)selector withBlock:(id)block {
	...
	_BlockInvocation *inv = nil;
	if (methodDescription.name) {
		NSMethodSignature *protoSig = [NSMethodSignature signatureWithObjCTypes:methodDescription.types];
		inv = [[_BlockInvocation alloc] initWithBlock:block methodSignature:protoSig];
	} else {
		inv = [[_BlockInvocation alloc] initWithBlock:block];
	}
...
// className + suffix
static Protocol *__classProtocol(Class _cls, NSString *suffix, NSString *description) {
	Class cls = _cls;
	while (cls) {
...
Protocol *__dataSourceProtocol(Class cls) {
	return __classProtocol(cls, @"DataSource", @"data source");
}

Protocol *__delegateProtocol(Class cls) {
	return __classProtocol(cls, @"Delegate", @"delegate");
}
