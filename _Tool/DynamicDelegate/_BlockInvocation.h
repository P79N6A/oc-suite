...
/** An `_BlockInvocation` is an Objective-C block call rendered static, that
 is, it is an action turned into an object. `_BlockInvocation` objects are used
 to store and forward closure invocations between objects, primarily by the
 `_DynamicDelegate` system.
 ...
 `_BlockInvocation` extremely useful, because blocks can be used to capture
 scope and a block invocation can be used to save it for later.

 Like `NSInvocation`, `_BlockInvocation` does not support invocations of
 methods with variadic arguments or union arguments.
 */
@interface _BlockInvocation : NSObject
...
/** @name Creating _BlockInvocation Objects */
...
/** Calls the receiver's block with the arguments from the given invocation,
 ...
 return type.
 @return NO if the buffer copies necessary for invocation failed, YES otherwise.
