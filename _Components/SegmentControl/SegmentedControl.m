...
@property (nonatomic, readwrite) CGFloat stringWidth; // Valid when fixed is set.
...
- (void)drawRect:(CGRect)rect {
    ...
    if (self.type == HMSegmentedControlTypeText) {
        [self.sectionTitles enumerateObjectsUsingBlock:^(id titleString, NSUInteger idx, BOOL *stop) {
            ...
            // TextInsets
            {
                CGRect backgroundLayerRect;
                if (idx == 0) {
                    backgroundLayerRect = CGRectMake(5, 5, self.segmentWidth-5, self.frame.size.height-5*2);
                } else {
                    backgroundLayerRect = CGRectMake(self.segmentWidth*idx+2.5f, 5, self.segmentWidth-5, self.frame.size.height-5*2);
                }
                
                BOOL isBackgroundLayerInit = NO;
                CAShapeLayer *backgroundLayer =  [CAShapeLayer layer];
                backgroundLayer.frame = backgroundLayerRect;
                if (self.titleButtonStyleBorderColor) {
                    backgroundLayer.borderWidth = 1.f;
                    backgroundLayer.borderColor = self.titleButtonStyleBorderColor.CGColor;
                    
                    isBackgroundLayerInit = YES;
                }
                if (self.titleButtonStyleBackgroundColor) {
                    backgroundLayer.backgroundColor = self.titleButtonStyleBackgroundColor.CGColor;
                    
                    isBackgroundLayerInit = YES;
                }
                if (self.titleButtonStyleCornerRadius > 0) {
                    backgroundLayer.cornerRadius = self.titleButtonStyleCornerRadius;
                    backgroundLayer.masksToBounds = YES;
                    
                    isBackgroundLayerInit = YES;
                }
                if (isBackgroundLayerInit) {
                    [self.scrollView.layer insertSublayer:backgroundLayer below:titleLayer];
                }
            }
