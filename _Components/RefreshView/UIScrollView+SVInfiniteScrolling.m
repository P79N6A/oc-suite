...
#pragma mark - UIScrollView ( InfiniteScrollingView )
...
- (void)addInfiniteScrollingWithActionHandler:(void (^)(void))actionHandler {
    ...
    if(!self.infiniteScrollingView) {
        InfiniteScrollingView *view = [[InfiniteScrollingView alloc] initWithFrame:CGRectMake(0, self.contentSize.height, self.bounds.size.width, InfiniteScrollingViewHeight)];
...
- (void)infiniteEndRefreshing {
    [self.infiniteScrollingView stopAnimating];
}

- (void)infiniteFailedRefreshing {
...
- (void)setShowsInfiniteScrolling:(BOOL)showsInfiniteScrolling {
    ...
    if(!showsInfiniteScrolling) {
      ...
      if (!self.infiniteScrollingView.isObserving) {
        ...
        self.infiniteScrollingView.frame = CGRectMake(0, self.contentSize.height, self.infiniteScrollingView.bounds.size.width, InfiniteScrollingViewHeight);
      }
    }
}
...
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {    
    if([keyPath isEqualToString:@"contentOffset"])
        ...
        [self layoutSubviews];
        self.frame = CGRectMake(0, self.scrollView.contentSize.height, self.bounds.size.width, InfiniteScrollingViewHeight);
    }
}
...
- (void)scrollViewDidScroll:(CGPoint)contentOffset {
    if(self.state != InfiniteScrollingStateLoading && self.enabled) {
        ...
        if(!self.scrollView.isDragging && self.state == InfiniteScrollingStateTriggered)
            self.state = InfiniteScrollingStateLoading;
        else if(contentOffset.y > scrollOffsetThreshold && self.state == InfiniteScrollingStateStopped && self.scrollView.isDragging)
...
- (void)setState:(InfiniteScrollingState)newState {
    ...
    if(previousState == InfiniteScrollingStateTriggered && newState == InfiniteScrollingStateLoading && self.infiniteScrollingHandler && self.enabled)
