...
- (void)adjustStateWithContentOffset
{
    ...
    if (self.scrollView.isDragging || self.inBindDragging) {
        // 普通 和 即将刷新 的临界点
	...
	switch (state) {
		case MJRefreshStateNormal: // 下拉可以刷新
        {
            ...
            if (MJRefreshStateRefreshing == oldState) {
                ...
                [UIView animateWithDuration:MJRefreshSlowAnimationDuration animations:^{
                    ...
                    if ([self.animateDelegate respondsToSelector:@selector(handleFinishAnimate)]) {
                        [self.animateDelegate handleFinishAnimate];
                    }
                }];
            } else {
                // 执行动画
		...
		case MJRefreshStateRefreshing: // 正在刷新中
        {
            ...
            if (!self.animateDelegate) {
                ...
                if ([self.animateDelegate respondsToSelector:@selector(handleRefreshingAnimate)]) {
                    [self.animateDelegate handleRefreshingAnimate];
                }
            }
