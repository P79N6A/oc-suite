...
- (MJRefreshHeaderView *)addHeaderWithTarget:(id)target action:(SEL)action animateDelegate:(id<MJRefreshHeaderViewDelegate>)delegate
{
    // 1.创建新的header
    if (!self.header) {
        MJRefreshHeaderView *header = [MJRefreshHeaderView header];
    ...
    self.header.beginRefreshingTaget = target;
    self.header.beginRefreshingAction = action;
    self.header.animateDelegate = delegate;
